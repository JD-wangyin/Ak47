 ...
 * 
 * Loading strategy (assume the prefix is 'xxx'):
 * 1. Check whether there {class_path}/xxx-test.yml, may be within the jar package. 
 *    If exists, load xxx-test.yml, if not, load {class_path}/xxx.yml, 
 *    if both are absent, throw an exception.
 * 2. Check whether there {$AK47_HOME}/conf/xxx.yml, load it if exists.
 * 3. Check whether there {$HOME}/.ak47/xxx.yml, load it if exists.
 * 
 * NOTICE: If a config value both in two conf file, 
 *         the after loaded will cover the before loaded.
 * 
 * WARNING: {class_path}/xxx-test.yml just be used in beta test, 
 *          should be invisible in official release.
 * 
 * 配置加载类
 * 
 * 加载顺序（后面覆盖前面）：
 * 
 *  1、  首先，检查是否有 getClassLoader()/xxx-test.yml，有可能在jar包内。
 *     若有，则加载 xxx-test.yml，若没有，则加载 xxx.yml，若两者都无，则抛异常。
 *     
    ...
    public static Map<String, Object> load(String prefix) throws IOException{
        ...
        if( null == testYmlIs ){
            ...
            if( null == ymlIs ){
                ...
                throw new Ak47RuntimeException(testYmlName+" and "+ymlName+" both NOT found.");
            }
            ...
            try{
                ...
                throw new Ak47RuntimeException("load "+testYmlName+" fail.", e);
            }finally{
                testYmlIs.close();
            }
        ...
        if( omap == null || omap.size() == 0 ){
            throw new Ak47RuntimeException("load "+testYmlName+" or "+ymlName+" fail.");
        }
        
        // app conf dir ( ak47/conf/xxx.yml )
        String appConfYmlPath = Ak47Env.AK47_HOME_CONF_DIR + File.separator + ymlName;
        ...
        if( !appConfYmlFile.exists() ){
            ...
            try{
                ...
                throw new Ak47RuntimeException("load "+appConfYmlPath+" fail.", e);
            }finally{
                fis.close();
            }
            ...
            if( null == newMap ){
                log.error("load {} fail.", appConfYmlPath);
                throw new Ak47RuntimeException("load "+appConfYmlPath+" fail. result is null.");
            }
        ...
        String homeYmlPath = Ak47Env.AK47_LOCAL_SHARE_DIR + File.separator + ymlName;
        ...
        if( !homeYmlFile.exists() ){
            ...
            try{
                ...
                throw new Ak47RuntimeException("load "+homeYmlPath+" fail.", e);
            }finally{
                fis.close();
            }
            ...
            if( null == newMap ){
                log.error("load {} fail.", homeYmlPath);
                throw new Ak47RuntimeException("load "+homeYmlPath+" fail. result is null.");
            }
